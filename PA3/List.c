...
void set(List L, long x){
	...
	if( index(L) < 0 ){
	    printf("List Index Error: calling set() on an undefined cursor\n");
	    exit(1);
    }

	if( (length(L) < 0) ){
		printf("List Error: calling set() on List with length() < 0\n");
		exit(1);
	}
...
void deleteFront(List L){ 
	...
	if( length(L) > 1 ) { // when there are nodes in the list deletefront
...
void delete(List L){
	...
	// node to be deleted is in the front
	if(L->front == N){
		L->front = N->next;  
	}
	else if(L->back == N)
